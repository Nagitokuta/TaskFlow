...
<div class="bg-white rounded-lg shadow border border-gray-200 p-6 max-w-2xl">
    <form method="POST" action="{{ route('tasks.update', $task) }}" class="space-y-4">
        ...
        <div class="flex gap-3 pt-2">
            <button type="submit" class="bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 font-medium">更新</button>
            <a href="{{ route('tasks.show', $task) }}" class="bg-gray-200 text-gray-800 py-2 px-4 rounded-md hover:bg-gray-300">キャンセル</a>
        </div>
    </form>
</div>
@endsection